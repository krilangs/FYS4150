...
void MetropolisSampling(int NSpins, int MCcycles, double Temperature, vec &ExpectationValues, int &Nconfigs, bool randomconfig, vec &Energies, vec &counter)
{
  ...
  if (MCcycles >= 7500){ // Hard coded the equilibrium state
...
void WriteAnalytical(ofstream& ofile){
    ...
    ofile << setw(20) << setprecision(8) << Mabs; // Mean magnetization
    ofile << setw(20) << setprecision(8) << Cv; // Specific heat Cv
    ofile << setw(15) << setprecision(8) << Xi; // Susceptibility
} // end output function

void WriteResultsto4b(ofstream& ofile, int MCcycles, double temperature, vec ExpectationValues)
{
  double norm = 1.0/(double (MCcycles));  // divided by  number of cycles
  double E_ExpectationValues = ExpectationValues(0)*norm;
  double E2_ExpectationValues = ExpectationValues(1)*norm;
  ...
  ofile << setw(20) << setprecision(8) << Mabs_ExpectationValues; // Mean magetization
  ofile << setw(20) << setprecision(8) << Evariance/temperature/temperature; // Specific heat Cv
