...
{
    /*
     * This function computes integrand in the six-dimensional integral for the quantum
     * mechanical expectation value of the correlation energy between two electrons which
     * repel each other via the classical Coulomb interaction
     * in Cartesian coordinates.
     *
     * x1, x2, y1, y2, z1, z2: Are the Cartesian coordinates for the vector r1 and r2
     * alpha: Constant in exponential term.
    */
...
double int_func_spherical(double r1, double r2, double theta1, double theta2, double phi1, double phi2)
{
     ...
     *
     * r1, r2: Radial coordinates
     * theta1, theta2, phi1, phi2: Angular coordinates
    */
...
{
    /*
     * This function calculates the integral in Cartesian coordinates
     * with brute force Gaussian-Legendre quadrature using Legendre polynomials.
     *
     * a: Lower integration limit
     * b: Upper integration limit
     * N: Number of grid points between a and b
     * alpha: Constant in exponential term
    */
...
{
    /*
     * This function calculates the integral in spherical coordinates
     * with improved Gaussian-Laguerre quadrature using Laguerre polynomials.
     *
     * a: Lower integration limit
     * b: Upper integration limit
     * N: Number of grid points between a and b
    ...
    */
    ...
    double *w_s = new double [N+1];
    double *w_theta = new double [N];
    double *w_phi = new double [N];

    double I = 0;
    gauss_laguerre(s, w_s, N+1, alpha);
    ...
    for (int m=0; m < N; m++){
    for (int n=0; n < N; n++){
        I += w_s[i] * w_s[j] * w_theta[k] * w_theta[l] * w_phi[m] * w_phi[n]
             * int_func_spherical(s[i],s[j], theta[k], theta[l], phi[m], phi[n])
...
pair<double, double> monte_carlo(double a, double b, double alpha, int N)
{
    /*
     * This function calculates the integral in Cartesian coordinates
     * with brute force Monte Carlo integration.
     *
     * a: Lower integration limit
     * b: Upper integration limit
     * N: Number of grid points between a and b
    ...
    */
    ...
    mt19937_64 generator(rd());   // Mersene Twister 19937 generator (64 bit)
    // Random distribution
...
pair<double, double> mc_improved(double alpha, int N)
{
    /*
     * This function calculates the integral in spherical coordinates
     * with imporved Monte Carlo integration using importance sampling.
     *
     * N: Number of grid points between a and b
     * alpha: Constant in exponential term
    */
...
pair<double, double> mc_parallization(double alpha, int N, int n_threads)
{
    /*
     * This function calculates the integral in spherical coordinates
     * with parallization of the Monte Carlo integration with a
     * given number of threads and compiler flag.
     *
     * N: Number of grid points between a and b
     * alpha: Constant in exponential term
     * n_threads: Number of threads to use in the parallization
    */
    double I;
    double Var;
    double func_val;
...
int main()
{
    int N;  // Number of grid points between a and b
    int n_threads;  // Number of threads to use in the parallization
    double a, b;  // Lower and upper limits of the integral
    double alpha = 2;  // Constant in exponential term
    double analytic = 5 * PI * PI / (16*16);  // Analytic answer to the integral
